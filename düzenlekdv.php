...
$sonuc=$sorgu->fetch();
if($_POST){
    $guncelleSorgu=$baglanti->prepare("UPDATE kdvler SET kdvorani=:kdvorani  WHERE id=:id");
    ...
    if ($guncelle) {
        ...
        swal("Başarılı", "Güncellleme başarılı şekilde gercekleşti", "success").then((value)=>{ window.location.href = "kdv"});
        </script>
        ';
...
?>
   ...
   <label class="control-label">% KDV Sabiti</label>
 <input type="text" id="ad" class="form-control" placeholder="Marka Adı" name="kdvorani" value="<?= $sonuc["kdvorani"] ?>">
                                                  </div>
                                            </div>
